...
void run(Run run) { UNUSED(run);
	...
	Fmt _fmt = fmt_init(pages_pinned(&fmt_pages)); Fmt * fmt = &_fmt;
	...
	Fmt _paths = fmt_init(pages_pinned(&paths_pages)); Fmt * paths = &_paths;
	...
	fmt_lit_z(paths, "build");          StrMut const build_dir  = paths->last;
	fmt_lit_z(paths, "build/main.asm"); StrMut const build_file = paths->last;
